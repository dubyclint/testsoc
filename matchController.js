// controllers/matchController.js - Match Request Controller for Supabase/PostgreSQL
...
export class MatchController {
  ...
  static async getMatchStats(req, res) {
    try {
      ...
      const stats = {
        ...
        accepted_matches: [...sentRequests, ...receivedRequests].filter(r => r.status === 'accepted').length,
        completed_matches: [...sentRequests, ...receivedRequests].filter(r => r.status === 'completed').length,
        success_rate: 0
      };

      if (stats.sent_requests > 0) {
        stats.success_rate = Math.round((stats.completed_matches / stats.sent_requests) * 100);
      }
  ...
  // Report inappropriate match behavior
  static async reportMatch(req, res) {
    try {
      const { requestId } = req.params;
